...
#pragma warning disable CA1826 // Запрет использования метода Enumerable для индексируемых коллекций
        var key = _dbContext.Model.FindEntityType(typeInfo)?.FindPrimaryKey()?.Properties.FirstOrDefault();
#pragma warning restore CA1826 // Запрет использования метода Enumerable для индексируемых коллекций
        if (key is null)
        {
            return;
        }
