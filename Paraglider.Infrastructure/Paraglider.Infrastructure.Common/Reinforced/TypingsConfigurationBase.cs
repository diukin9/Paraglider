...
namespace Paraglider.Infrastructure.Reinforced
{
    public static class TypingsConfigurationBase
    {
        public static void Configure(
            ConfigurationBuilder builder,
            Assembly assembly,
            string typingsNamespace
        )
        {
            var types = assembly
                .GetTypes()
                .Where(type => IsViewModel(type.Namespace, typingsNamespace));

            builder.Global(
                config => config.UseModules().AutoOptionalProperties().CamelCaseForProperties()
            );
